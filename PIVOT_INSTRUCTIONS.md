...
La conception détaillée de Lane 2+ (kernels, tables, budgets de cycles) **n'a pas été relancée** — le cahier des charges est en cours de révision côté utilisateur (sections 2, 3, 4.1-4.3, 6, 7, 9.2, 10, 11.1-11.2, 12.2, 12.7) pour retirer toute référence ARM et clarifier le chemin principal (notamment : le playfield asynchrone pour les briques, déjà le palier de repli, devient probablement le seul chemin — à confirmer par la révision, pas anticipé ici). Toute conception de ce périmètre attend cette version à jour.
