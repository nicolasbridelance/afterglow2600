...
**Mise à jour 2026-08-31 (suite) — la version parallèle est arrivée, réconciliée plutôt qu'écrasée.** L'autre session avait effectivement produit son propre Draft v8 (`cahier-des-charges-atari2600-portrait-vivant.md`, déposé à la racine du repo par l'utilisateur). Plutôt que de choisir un camp, cette session a lu les deux versions en entier et intégré dans `cahier_des_charges.md` ce qui améliorait réellement le document : une reformulation positive du pitch (§1, "ce qu'un développeur de 1982 aurait fait avec les outils modernes"), une note historique sur le DPC d'Activision comme alternative d'époque à l'ARM (§2), et surtout une correction factuelle importante — l'ambition d'origine posée en §12.1 était "un dégradé par **rangée**", pas par brique individuelle ; le per-brique était un embellissement du draft v7 inspiré du précédent SpiceWare, pas l'ambition de départ. Cette nuance a été propagée dans §4.2, §9, §12.10.4 : le recul de densité réel est plus contenu que ce que la première rédaction du pivot laissait entendre, mais reste réel (le multiplicateur de coût d'écriture, lui, ne disparaît pas). L'autre version proposait aussi de traiter la perte comme quasi nulle ("on revient à l'ambition de départ, pas à un vrai sacrifice") — ce jugement n'a **pas** été repris tel quel, jugé trop optimiste au vu du coût d'écriture réel (§12.7). Les mesures de spikes ont aussi été inlinées directement dans §9.2 (plus besoin de sauter vers `backlog.md` pour les chiffres bruts). Le fichier `cahier-des-charges-atari2600-portrait-vivant.md` reste dans le repo, non supprimé — voir avec l'utilisateur s'il doit être archivé maintenant que sa substance est absorbée.
